...
class Lista
{
    public:
        ...
        int Insertar(T Dir, int posicion);
        int Agregar(T Dir);
        int Localiza(T Dir);
        T Recupera(int posicion);
        void Siguiente();
        void Anterior();
        void Localiza_Suprime(T Dir);
        void Imprimir();
        void Anula();
    ...
    private:
        ...
        Nodo<T>* LocalizaNodo(T Dir, int *posicion);
...
template <class T>
int Lista<T>::Insertar(T Dir, int posicion)
{
    Nodo<T>* nodo= new Nodo<T>(Dir);
...
template <class T>
int Lista<T>::Agregar(T Dir)
{
    return Insertar(Dir, size);
}

template <class T>
int Lista<T>::Localiza(T Dir)
{
   int pos=0;
   LocalizaNodo(Dir, &pos);
   return pos;
}
